...
public class HistoryPlayerImpl
  ...
  public HistoryPlayerImpl() {
    this.historyPlayerListeners = new ListenersManager<>();
    this.tagsToRegisterLiveValue = new ArrayList<>();
    this.tagsToRegisterLiveValueLock = new ReentrantLock();
    this.supervisionEventsToRegister = new ArrayList<>();
  ...
  public void registerTagUpdateListener(final TagUpdateListener tagUpdateListener, final Long tagId, final Tag currentRealtimeValue) {
    if (isHistoryPlayerActive()) {
      ...
      final HistoryTagValueUpdateImpl currentValue;
      if (currentRealtimeValue != null) {
        currentValue = new HistoryTagValueUpdateImpl(
        ...
        if (currentRealtimeValue.getType() != null) {
          currentValue.setValueClassName(currentRealtimeValue.getType().getName());
        }
      }
      else {
        currentValue = new HistoryTagValueUpdateImpl(tagId);
      }
  ...
  private void unregisterTagsFromHistoryStore(final Collection<Long> tagIds) {
    final List<HistoryUpdateId> removedDataIds = new ArrayList<>(tagIds.size());
  ...
  public void unregisterSupervisionListener(final SupervisionEntity entity, final SupervisionListener listener) {
    ...
    final List<HistoryUpdateId> removedDataIds = new ArrayList<>(removedIds.size());
  ...
  public void beginLoading() {
    ...
    try {
      ...
      if (!filteredTags.isEmpty()) {
        ...
        final List<HistoryUpdateId> tagsToLoad = new ArrayList<>();
