...
public class HistoricAlarmQuery {
  ...
  public HistoricAlarmQuery timestamp(Timestamp timestamp) {
    Timestamp utcTimeStamp = TimeZoneUtil.convertDateTimezoneToUtc(timestamp);
    builder.and(QAlarm.alarm.timestamp.eq(utcTimeStamp));
    return this;
  }
  ...
  public HistoricAlarmQuery between(Timestamp start, Timestamp end) {
    Timestamp utcStart = TimeZoneUtil.convertDateTimezoneToUtc(start);
    Timestamp utcEnd = TimeZoneUtil.convertDateTimezoneToUtc(end);
    builder.and(QAlarm.alarm.timestamp.between(utcStart, utcEnd));
    return this;
  }
